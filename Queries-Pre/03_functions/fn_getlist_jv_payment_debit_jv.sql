CREATE OR REPLACE FUNCTION fn_getlist_jv_payment_debit_jv(
...
BEGIN 
     create temp table if not exists temp_union as 
        ...
        where 
            ...
            and (tjd.accountid like '20310%' or tjd.accountid = '1070020') 
        ; 
 
	create temp table if not exists temp_outstanding as 
		select tmp.jv_id as jv_id, tmp.jv_detailid as jv_line, 
			sum(tmp.foreignamount) as ForeignAmountOutstanding, sum(tmp.idramount) 
				 as IDRAmountOutstanding  
			from temp_union tmp 
    ...
    return query
	    select 
	        ...
	        ( 
	           CASE tjd.accountid  
	           WHEN '1070020' THEN 
	               coalesce(tmp.idramountoutstanding,0) 
	           ELSE 
	               coalesce(-tmp.idramountoutstanding,0) 
	           END 
	        ) as idramountoutstanding, 
	        ( 
	           CASE tjd.accountid  
	           WHEN '1070020' THEN 
	               coalesce(tmp.foreignamountoutstanding,0) 
	           ELSE 
	               coalesce(-tmp.foreignamountoutstanding,0) 
	           END 
	        ) as foreignamountoutstanding, 
	        ...
	        tj.tbudgetid, (select tb.name from transaction_budget tb where tb.tbudgetid = tj.tbudgetid)
	                as tbudgetname, tjd.tbudget_detailid as tbudget_detailid 
	    from 
	        transaction_journal_detail tjd 
