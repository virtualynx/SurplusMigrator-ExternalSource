...
CREATE materialized VIEW view_updateposttx AS 
	SELECT 
		...
		(('01'::text || to_char(tm1.mo_book_date, 'MMYYYY'::text)) || '-'::text) || to_char(last_day(tm1.mo_book_date::timestamp with time zone)::timestamp with time zone, 'DDMMYYYY'::text) AS period,
	...
	FROM 
		tt_mo1 tm1
		JOIN mg_channel mcn ON tm1.channel_code::text = mcn.channel_code::text
		JOIN st_reservations sr1 ON sr1.row_id::text = tm1.row_id_res::text
		JOIN st_reservation_matrix1 srm1 ON srm1.row_id::text = tm1.row_id_matrix1::text
		LEFT JOIN mp_film mf ON tm1.film_poc::text = mf.film_poc::text AND tm1.row_id_poc::text = mf.row_id::text
		LEFT JOIN tp_cbs_dps1 tcd1 ON tm1.row_id_slot::text = tcd1.row_id::text
		LEFT JOIN mt_prod1 mp1 ON tm1.prod_code::text = mp1.prod_code::text AND tm1.prod_version::text = mp1.prod_version::text
